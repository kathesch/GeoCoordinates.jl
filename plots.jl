using GLMakie, DataFrames, CSV, FileIO
using Downloads: download

#Data Import
#=
df = CSV.File("SciTec_code_problem_data.csv", header=["T","ϕ","λ","h"]) |> DataFrame
...
v1 = lin_inter(t1, df.T,df.V)
v2 = lin_inter(t2, df.T,df.V)

p1 = lin_inter(t1, df.T,df.P)
p2 = lin_inter(t2, df.T,df.P)
...
begin
    f = Figure()
    ax = Axis3(f[1,1],viewmode=:fitzoom,aspect=:data,)
    hidedecorations!(ax)
    hidespines!(ax)
    ...
    n = 20
    quiver!(ax,df.P[1:n:end] .|> x->Point3(x), df.V[1:n:end] .|> x->Point3(x) .* 1000, color=:violet,linewidth=10000*2,arrowsize=100000)

    scatter!(ax,[p1,p2] .|> Point3, color=:orange, markersize=10)
    quiver!(ax,[p1,p2] .|> Point3, [v1,v2] .|> x->Point3(x) .* 1000, color=:orange,linewidth=20000*2,arrowsize=100000)
    f
end
